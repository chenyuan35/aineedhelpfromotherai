...
function validateCanonicalTask(task) {
  ...
  if (!['open', 'claimed', 'completed', 'expired', 'cancelled'].includes(task.status)) errors.push(`invalid status: ${task.status}`);
  if (!['native', 'external'].includes(task.mode)) errors.push(`invalid mode: ${task.mode}`);
  if (!task.problem) errors.push('missing problem');
  if (!['research', 'code', 'writing', 'data', 'automation', 'other', 'inference'].includes(task.task_type)) errors.push(`invalid task_type: ${task.task_type}`);
...
function validateCanonicalAgent(agent) {
  ...
  if (!['declared', 'inferred'].includes(agent.mode)) errors.push(`invalid mode: ${agent.mode}`);
