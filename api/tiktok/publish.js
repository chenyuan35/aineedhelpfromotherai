...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(body.consent!==true)throw new TikTokError('Explicit user consent is required before sending content to TikTok.',{source:'server',code:'publish_consent_required',httpStatus:400});
    if(mode==='upload'){
    ...
    const creator=await creatorInfo(accessToken);const privacy=String(body.privacy_level||'').trim();const privacyOptions=Array.isArray(creator.privacy_level_options)?creator.privacy_level_options:[];const duration=Number(body.video_duration_sec);const maxDuration=Number(creator.max_video_post_duration_sec||0);
    ...
    if(!Number.isFinite(duration)||duration<=0)throw new TikTokError('TikTok requires the selected video duration before Direct Post.',{source:'server',code:'video_duration_required',httpStatus:400});
    if(maxDuration&&duration>maxDuration)throw new TikTokError('The selected video exceeds the connected creator maximum duration.',{source:'server',code:'video_too_long',httpStatus:400});
    const brandOrganic=body.brand_organic_toggle===true;const brandContent=body.brand_content_toggle===true;const isAigc=body.is_aigc===true;
    if(body.commercial_content===true&&!brandOrganic&&!brandContent)throw new TikTokError('Choose Your brand, Branded content, or both for commercial content.',{source:'server',code:'commercial_disclosure_required',httpStatus:400});
    if(brandContent&&privacy==='SELF_ONLY')throw new TikTokError('Branded content cannot use Only me privacy.',{source:'server',code:'branded_content_private',httpStatus:400});
    ...
    const tiktok=await tiktokJson(DIRECT_POST_URL,accessToken,{post_info:{title:caption,privacy_level:privacy,disable_comment:!allowComment,disable_duet:!allowDuet,disable_stitch:!allowStitch,video_cover_timestamp_ms:1000,brand_content_toggle:brandContent,brand_organic_toggle:brandOrganic,is_aigc:isAigc},source_info:{source:'FILE_UPLOAD',video_size:fileSize,chunk_size:chunkSize,total_chunk_count:totalChunkCount}});
    return sendJson(res,200,{ok:true,mode,publish_id:tiktok.data&&tiktok.data.publish_id,upload_url:tiktok.data&&tiktok.data.upload_url,chunk_size:chunkSize,total_chunk_count:totalChunkCount,tiktok});
