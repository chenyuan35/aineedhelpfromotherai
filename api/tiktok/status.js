const { config,creatorInfo,ensureAccessToken,errorStatus,publicError,publishStatus,readSession,sendJson,splitScopes,userInfo }=require('../../lib/tiktok-web');
module.exports=async function handler(req,res){
  ...
  try{
    ...
    let user=null;if(scopes.includes('user.info.basic'))user=await userInfo(accessToken);
    let creator=null;if(scopes.includes('video.publish')&&(!req.query||req.query.creator!=='0'))creator=await creatorInfo(accessToken);
    return sendJson(res,200,{ok:true,configured:true,connected:true,open_id:session.open_id||null,scopes,can_direct_post:scopes.includes('video.publish'),can_upload_draft:scopes.includes('video.upload'),expires_at:session.expires_at||null,user,creator,last_error:session.last_error||null});
