...
17. **NEXT SESSION — Holafly Always On packet.** Verify the cheapest legitimate direct purchase that activates Always On, China/Hong Kong/Singapore coverage, device-transfer/deletion limits, monthly refresh behavior and current independent reliability. Keep it in Data eSIM; no auto-publication.
18. **BACKLOG / separate review — second observer/source-change role.** `codex-vps` already has a Relay-history role; do not repurpose it merely to satisfy a two-host topology. The existing trial host already runs the reviewed phone-source timer, so a second machine needs a concrete reliability/value reason before assignment.
19. **ONGOING — Relay data accrual.** No search-led expansion without evidence.
20. **WAIT — TikTok review and authority batch 2.** Do not disturb external waits before their triggers.
