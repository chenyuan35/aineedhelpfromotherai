...
| P1 Initial tool inventory | Publish a useful, low-cost starter portfolio | DONE | 14 live tool pages across calculators, image tools, AI quota/reset tools and the Relay Exit Risk Checker community-risk pilot | Completed |
| P2 Discovery & indexing | Make Google discover and index the current utility cohort | IN PROGRESS | GSC connected; 15 priority URLs tracked; tracker remains 4 indexed / 11 not indexed after Sep 13 requests. `/tools/`, Cursor reset and image compressor were crawled successfully and indexed alongside the homepage. URL normalization is fixed. Live sitemap now contains 20 URLs after the Sep 14 relay-risk release; the existing 15-URL tracker predates that new URL. | Tool cohort starts receiving its own settled impressions, with indexing continuing beyond the first priority pages |
...
| P5 Controlled expansion | Add 1–2 evidence-backed tools per round | QUEUED / ONE-OFF PILOT VERIFIED | Candidate feed comes from VPS radar + GSC + Ubersuggest + official docs/community evidence. Relay Exit Risk was the explicit distinct-job exception and is now production-verified as v2 through PR #42–#44 with evidence-driven scoring, third-party ingestion, stored snapshots and time-bounded community forecasts. This does not authorize a broad batch | Each round ships, indexes and is measured before the next broad round |
...
4. ACTIVE — Keep the 15-URL GSC indexing tracker running. Current state is 4 indexed and 11 not indexed. The homepage, `/tools/`, Cursor reset and image compressor are indexed; do not spam repeated recrawl requests for the remaining 11. The Sep 14 relay-risk URL is outside this existing tracker and should not trigger immediate request-indexing churn.
...
8. WAITING FOR SIGNAL — Select the next broad 1–2 tools only from combined evidence: GSC signal + radar/autocomplete + official rules/community pain + keyword metrics when available. The Sep 14 relay-risk page was an explicit user-directed one-off pilot, not a restart of broad expansion.
...
23. MONITOR — Continue watching the remaining 11 tracked URLs. Do not manually request-index every remaining URL unless evidence shows discovery has stalled after the current crawl/index wave settles.
...
26. NEXT — Reposition the homepage so its distinctive job is obvious on first view. First priority: AI reset / limits / usage with separate entries for Cursor, GitHub Copilot, Manus, Replit and Bolt, plus Codex and Claude when those distinct tools are validated and live. Second priority: Relay Exit Risk as the next signature section. The homepage may visually group these under one AI-user utility identity, but every click must still lead to its own independent tool/page; do not merge provider logic or create misleading dead entries. Generic calculators and image utilities remain available but move below the two signature AI sections.
27. PREPARED / ASSET DEPENDENCY — Build a reusable original homepage character system around a satirical chubby office boss: walking near the desk, sitting/counting cash, and optionally speaking via HTML overlay text such as `良心中转，满血，不坑人`. The preferred asset path is a user-generated 5–6 second fixed-camera loop, then post-process it for web delivery (cropping, seamless loop, WebM/MP4 compression, poster frame, mobile/static fallback). Do not bake provider names, UI or speech text into the source video. Relay Risk can reuse the same character language as a second-section teaser, while the detailed tool page remains driven by its actual Risk Index. This visual work is a homepage identity task, not evidence about any relay and not permission for a broad product batch.
