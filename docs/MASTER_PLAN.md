...
19. **NEXT SESSION — PR #233 disposition review.** Reconcile the five-public-URL Keep-Alive Assistant with the KEEP instruction and product/publication gates; end in HOLD / REWORK / CLOSE, not an automatic merge.
20. **BACKLOG / separate review — second observer/source-change role.** `codex-vps` already has a Relay-history role; do not repurpose it merely to satisfy a two-host topology. The existing trial host already runs the reviewed phone-source timer, so a second machine needs a concrete reliability/value reason before assignment.
21. **ONGOING — Relay data accrual.** No search-led expansion without evidence.
22. **WAIT — TikTok review and authority batch 2.** Do not disturb external waits before their triggers.
