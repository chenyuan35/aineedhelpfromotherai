...
|---|---|---|---|---|
| P0 Foundation | Stable static site, safe deploy flow, durable handoff | DONE | GitHub→Vercel flow, CI/Eval Gate, `AGENTS.md`, `PROJECT_CONTEXT.md`, workflow docs. PR #40 additionally repaired the apex-domain API rewrite path used by the new shared-state tool and restored the reasoning handler as valid UTF-8 before the VPS backend was reloaded | Completed |
...
| P5 Controlled expansion | Add 1–2 evidence-backed tools per round | QUEUED / ONE-OFF PILOT SHIPPED | Candidate feed comes from VPS radar + GSC + Ubersuggest + official docs/community evidence. On Sep 14 the user explicitly authorized one distinct-job exception: Relay Exit Risk Checker, based on repeated relay shutdown/prepayment/model-substitution discussion. It is live and production-verified; this does not authorize a broad batch. | Each round ships, indexes and is measured before the next broad round |
...
24. DONE — Relay Exit Risk Checker one-off pilot shipped in PR #39. It provides English community exit/shutdown-risk voting, model-dilution sentiment, five satirical risk-stage animations, prepaid-balance exposure calculation, share links and external specialist verification links. PR #40 fixed two release-path defects found during production verification: a corrupted reasoning handler from the Git-data upload path and the Vercel trailing-slash API proxy fall-through. CI/Eval Gate and Preview passed; Vercel production and the VPS backend were deployed; direct tunnel and apex-domain API health/stats/relay-risk returned JSON; a real PostgreSQL vote/update/readback smoke test passed and the smoke row was deleted. This is a product/infra release, not an AIR provider benchmark or authority outcome.
25. MONITOR — Let Relay Exit Risk Checker gather real usage and community votes without manufacturing an algorithmic "runaway probability." Keep community sentiment separate from independently verified incidents/evidence, and do not expand it into a large relay directory until usage and evidence quality justify that work.
...
- For the Relay Exit Risk Checker: review real vote volume, abuse/spam behavior and referral/search demand before adding more reputation features; do not interpret a tiny sample as evidence about a relay.
