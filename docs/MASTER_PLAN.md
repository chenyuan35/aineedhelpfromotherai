...
21. **NEXT SESSION — PR #253 normalized Phone data foundation release.** Rebase on current `main`, rerun Phone canonical/staging isolation checks, resolve or correctly classify the repeated Vercel Preview failure, then merge if the release gate passes. After merge, use the standard importer for the three newly admitted routes.
22. **BACKLOG / separate review — second observer/source-change role.** `codex-vps` already has a Relay-history role; do not repurpose it merely to satisfy a two-host topology. The existing trial host already runs the reviewed phone-source timer, so a second machine needs a concrete reliability/value reason before assignment.
23. **ONGOING — Relay data accrual.** No search-led expansion without evidence.
24. **WAIT — TikTok review and authority batch 2.** Do not disturb external waits before their triggers.
