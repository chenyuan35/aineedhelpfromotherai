...
26. DONE — PR #47 repositioned the homepage so its distinctive jobs are obvious on first view. AI reset / limits / usage is now the primary hero and tool section with live independent entries for Cursor, GitHub Copilot, Manus, Replit and Bolt; Codex and Claude are shown only as upcoming directions, not live/dead links. Relay Exit Risk is the second signature section and retains the explicit 0–100 risk-index / not-a-fraud-probability language. Generic calculators and image utilities remain available below the signature sections. Vercel Preview, Eval Gate and production Chromium desktop/mobile/full-page checks passed.
27. DONE — PR #47 shipped the reusable satirical chubby-office-boss homepage character system. The hero and Relay section use lightweight poster + silent MP4 loops with HTML overlay text (including `良心中转，满血，不坑人`) so provider names/UI are not baked into the media. Production serves the assets from Vercel/CDN; the two MP4 loops are tiny static files and add no VPS streaming/transcoding workload. The Relay section explicitly labels `SATIRE ≠ EVIDENCE`, and the detailed Relay tool remains driven by its actual Risk Index. The static build was also fixed to preserve the curated homepage and ship `home.css`/`media/` instead of regenerating the old homepage.
