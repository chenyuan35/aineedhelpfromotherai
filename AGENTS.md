...
6. After the GitHub sources above are understood, read the latest end-of-day closeout in the Notion page `aineedhelpfromotherai — Daily Project Journal` when prior-day execution detail is useful. Treat it as chronological context only; it never overrides GitHub `main` or verified production.
...
- Treat the latest Notion end-of-day closeout as yesterday's execution diary only after reading GitHub `main`; use it to understand what was attempted, failed, blocked or deferred without allowing it to override canonical state.
