...
for (const marker of ['TIKTOK_CLIENT_KEY','TIKTOK_CLIENT_SECRET','TIKTOK_REDIRECT_URI','aes-256-gcm','https://www.tiktok.com/v2/auth/authorize/','https://open.tiktokapis.com/v2/oauth/token/','creator_info/query','status/fetch']) if (!shared.includes(marker)) throw new Error(`Shared TikTok module missing: ${marker}`);
const publish = readFileSync('api/tiktok/publish.js', 'utf8');
for (const marker of ['video.publish','video.upload','FILE_UPLOAD','privacy_level_options','publish/video/init']) if (!publish.includes(marker)) throw new Error(`Publish handler missing: ${marker}`);
