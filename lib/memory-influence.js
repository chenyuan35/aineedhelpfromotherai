...
// Enrich resolve-cache hints with MIS values
function enrich() {
  const scores = compute();
  const cache = resolveCache.getCache();
  let changed = false;

  for (const s of scores) {
    // Match by reasoning_id in cache or by memory_id
    for (const [taskId, hint] of Object.entries(cache.hints || {})) {
      if (hint.reasoning_id === s.memory_id || taskId === s.memory_id) {
        if (hint.mis !== s.mis || hint.influence_data !== undefined) {
          hint.mis = s.mis;
          hint.influence_data = {
            success_delta: s.success_delta,
            latency_improvement: s.latency_improvement,
            hallucination_reduction: s.hallucination_reduction,
            replay_runs_with: s.total_runs_with,
            replay_runs_without: s.total_runs_without_comparison,
          };
          changed = true;

          // Adjust score based on MIS
          const misBoost = Math.max(-1, Math.min(1, s.mis * 2));
          hint.score = Math.max(-3, Math.min(3, (hint.score || 1.0) + misBoost * 0.3));
          if (hint.score > 0.5 && hint.status === 'decaying') hint.status = 'active';
          if (hint.score < -0.3 && hint.status === 'active') hint.status = 'decaying';
        }
        break;
      }
    }
  ...
  if (changed) {
    ...
    require('./resolve-cache').save ? undefined : null;
    // Direct save via the module's internals
    const fs = require('fs');
    const path = require('path');
    const cp = path.join(__dirname, '..', 'data', 'resolve-cache.json');
    fs.writeFileSync(cp, JSON.stringify(cache, null, 2));
  }
  return { enriched: changed, scores_applied: scores.length };
}
