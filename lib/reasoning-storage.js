...
// LOCAL/DEMO ONLY. Never set DEV_REASONING_FALLBACK on Render: it bypasses
// PostgreSQL + commit-log governance (no audited write path, data lost on restart).
const DEV_FALLBACK = process.env.DEV_REASONING_FALLBACK === 'true';
const inMemoryROs = DEV_FALLBACK ? require('./reasoning-seeds') : [];
...
async function saveReasoning(ro) {
  ...
  const db = getPool();
  if (!db) {
    // DEV_FALLBACK: store in-memory (local/demo only; never enable on Render)
    if (DEV_FALLBACK) {
      inMemoryROs.push(ro);
      return ro;
    }
