...
async function searchReasoning(params) {
  ...
  // Full-text search on problem_statement and solution
  if (params.problem_statement) {
    // Convert search terms to tsquery format
    const terms = params.problem_statement
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, '')
      .split(/\s+/)
      .filter(t => t.length > 2)
    ...
    if (terms) {
      conditions.push(`(
        to_tsvector('english', problem_statement) @@ to_tsquery('english', $${idx})
        OR to_tsvector('english', COALESCE(solution->>'summary', '')) @@ to_tsquery('english', $${idx})
        OR problem_statement ILIKE $${idx + 1}
        OR COALESCE(solution->>'summary', '') ILIKE $${idx + 1}
      )`);
      values.push(terms.replace(/&/g, ' & '));
      values.push(`%${params.problem_statement}%`);
      idx += 2;
    }
  }
  ...
  // Rank by: full-text match > solution exists > success rate > recency
  const hasTextSearch = params.problem_statement;
  const query = `SELECT id, problem_id, problem_statement, context, solution, meta, created_at,
      CASE
        WHEN to_tsvector('english', problem_statement) @@ to_tsquery('english', $1) THEN 3
        WHEN problem_statement ILIKE $2 THEN 2
        ELSE 0
      END as search_rank
  ...
  const queryParams = hasTextSearch
    ? [
        params.problem_statement.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(t => t.length > 2).join(' & '),
