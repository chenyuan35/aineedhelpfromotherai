...
// Development fallback: in-memory reasoning objects when no DB is configured
const DEV_FALLBACK = process.env.DEV_REASONING_FALLBACK === 'true';
const inMemoryROs = DEV_FALLBACK ? [
  {
    id: 'RO_DEMO_1',
    problem_id: 'PROB_DEMO_1',
    problem_statement: 'example problem for cache test',
    context: { domain: 'general', difficulty: 'intermediate' },
    attempts: [{ agent_id: 'demo-agent', outcome: 'success' }],
    solution: { summary: 'Demo solution that avoids token use', key_insights: ['Use demo fallback'], consensus_score: 0.95 },
    meta: { total_attempts: 3, success_rate: 0.9, total_tokens: 100 }
  }
] : [];
...
async function ensureTable() {
  ...
  try {
    ...
    `);
    // v0.3: Runtime semantics fields
    await client.query(`
      ALTER TABLE reasoning_objects ADD COLUMN IF NOT EXISTS parent_run_id VARCHAR(64);
    `);
    await client.query(`
      ALTER TABLE reasoning_objects ADD COLUMN IF NOT EXISTS evidence_refs JSONB DEFAULT '[]';
    `);
    tableReady = true;
  } finally {
    client.release();
...
async function saveReasoning(ro) {
  ...
  if (!db) return null;
  await db.query(`
    INSERT INTO reasoning_objects (id, problem_id, problem_statement, context, attempts, solution, meta, parent_run_id, evidence_refs, updated_at)
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      meta = EXCLUDED.meta,
      parent_run_id = COALESCE(EXCLUDED.parent_run_id, reasoning_objects.parent_run_id),
      evidence_refs = CASE
        WHEN jsonb_array_length(EXCLUDED.evidence_refs) > 0 THEN EXCLUDED.evidence_refs
        ELSE reasoning_objects.evidence_refs
      END,
      updated_at = NOW()
  `, [
    ro.id,
    ...
    ro.parent_run_id || null,
    JSON.stringify(ro.evidence_refs || []),
  ]);
}
...
async function searchReasoning(params) {
  ...
  if (params.problem_statement) {
    const raw = params.problem_statement.toLowerCase();
    // Generate BOTH stripped and original terms to match stored text with punctuation
    const stripped = raw.replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(t => t.length > 2);
    const original = raw.split(/\s+/).filter(t => t.length > 2);
    // Merge deduplicated: prefer original (preserves punctuation for ILIKE)
    const termSet = new Set();
    [...original, ...stripped].forEach(t => termSet.add(t));
...
async function resolveReasoning(params) {
  ...
  // Demo fallback when DB not configured: simple in-memory search
  if (!db && DEV_FALLBACK) {
    const candidates = inMemoryROs.filter(ro => (ro.problem_statement || '').toLowerCase().includes(problem));
    if (candidates.length === 0) return { hit: false, reason: 'no_match' };
    const scored = candidates.map(full => ({
      id: full.id,
      problem_statement: full.problem_statement,
      solution: full.solution,
      context: full.context,
      meta: full.meta,
      quality_score: calculateQualityScore(full),
      success_rate: full.meta?.success_rate || 0,
      consensus_score: full.solution?.consensus_score || 0
    })).sort((a, b) => b.quality_score - a.quality_score);

    const best = scored[0];
    if (best.quality_score < 50) return { hit: false, reason: 'low_confidence', best_match: best.id, quality_score: best.quality_score };
    ...
    return {
      ...
      estimated_token_savings: estimatedTokens,
      message: `Demo cache HIT. Using ${best.id}`
    };
  }

  if (!db) return { hit: false, reason: 'db_unavailable' };
  ...
  // Cache hit threshold: quality score >= 30 (out of 100) AND has solution
  const isHit = best.quality_score >= 30 && best.solution;
...
async function failureCheck(params) {
  await ensureTable();
  const db = getPool();
  if (!db) return { risk_score: 0, warnings: [], message: 'Database unavailable' };
...
async function getProvenance(reasoningId) {
  ...
  const roUrl = `https://api.aineedhelpfromotherai.com/api/reasoning/${ro.id}`;

  // Standard provenance format per Reasoning Commons Protocol v0.2
  // Format: [RO-xxx](url) with consensus % and verification count
  const consensusPercent = consensusScore !== null ? Math.round(consensusScore * 100) : null;
  
  // Standard markdown format for AI to include in outputs
  const provenance_markdown = consensusPercent !== null
    ? `> 基于推理对象 [RO-${ro.id}](${roUrl})，共识度 ${consensusPercent}% (${verificationCount} 个验证)`
    : `> 基于推理对象 [RO-${ro.id}](${roUrl}) (${verificationCount} 个验证)`;

  const provenance = {
    // Standard fields (per Reasoning Commons Protocol)
    reasoning_id: ro.id,
    ...
    // Provenance blocks in different formats
    provenance_markdown: provenance_markdown,
    provenance_compact: `RO-${ro.id}(${consensusPercent || '?'}%)`,
    
    // Metadata
    problem_statement: ro.problem_statement.slice(0, 150),
    solution_summary: ro.solution?.summary?.slice(0, 150) || '',
    domain: ro.context?.domain || 'general',
    difficulty: ro.context?.difficulty || 'unknown',
    success_rate: ro.meta?.success_rate || 0,
...
function trackResolve(hit, problemStatement) {
  ...
  resolveLog = resolveLog.filter(e => now - e.ts < 3600000).slice(-1000);
}
function clearResolveLog() { resolveLog = []; }
