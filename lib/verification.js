...
function getStats() {
  ...
  for (const [id, m] of mems) {
    ...
    hints[id] = {
      ...
      effective_weight: TIER_WEIGHT[m.tier] * getDecayMultiplier(m.verified_at),
  ...
  return { total_tracked: mems.length, hints, ...weightSums, quarantine_candidates: quarantine, avg_replay_count: mems.length > 0 ? (mems.reduce((a, [, m]) => a + (m.replay_count || 0), 0) / mems.length).toFixed(1) : 0, avg_sandbox_pass_rate: mems.length > 0 ? (mems.reduce((a, [, m]) => a + (m.sandbox_runs > 0 ? m.sandbox_passes / m.sandbox_runs : 0), 0) / mems.length * 100).toFixed(0) : 0 };
}

module.exports = { TIERS, TIER_ORDER, TIER_WEIGHT, recordReplayConfirm, recordSandboxResult, recordProductionConfirm, getVerificationInfo, getEffectiveWeight, getDecayMultiplier, getDecayLabel, getStats, registerCapabilities };
