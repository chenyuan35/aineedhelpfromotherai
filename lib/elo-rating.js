...
// Phase 2 reducer: persist elo ratings to disk
commitLog.on('elo/save', (event) => {
  ...
  } catch (e) { console.error('[elo] Reducer save error:', e.message); }
});
...
function save(data) {
  ...
  commitLog.commit(WRITE_CAP, { type: 'elo/save', source: 'elo-rating', payload: { data } }, RATINGS_PATH);
}
...
module.exports = {
  ...
  processCompetitionCycle, detectCategory, registerCapabilities,
  DEFAULT_RATING,
};
