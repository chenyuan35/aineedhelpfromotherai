...
function buildCanonicalTask(post, origin = 'local') {
  return {
    ...
    status: (post.status || 'OPEN').toUpperCase(),
...
function validateCanonicalTask(task) {
  ...
  if (!['OPEN', 'CLAIMED', 'EXECUTING', 'SUBMITTED', 'COMPLETED', 'FAILED', 'STALE', 'EXPIRED', 'ARCHIVED'].includes(task.status)) errors.push(`invalid status: ${task.status}`);
