...
const translator = require('./event-translator');
...
const _ring = []; // circular buffer — last N flushed events for projection
const RING_MAX = 500;
...
function flushPending() {
  ...
  for (const evt of _pending) {
    ...
    // Translate internal event type to AI-native narrative
    const { narrative, narrative_action, severity } = translator.translate(evt.type, evt.data);
    base.narrative = narrative;
    base.narrative_action = narrative_action;
...
function emit(type, data = {}) {
  // Always capture to ring buffer for projection (independent of SSE clients)
  const { narrative, narrative_action, severity } = translator.translate(type, data);
  _ring.push({
    type,
    data: { ...data, narrative, narrative_action, ...(severity ? { severity } : {}) },
    aggregated: null,
    flushed_at: Date.now(),
  });
...
function getRecentEvents(count = 100) {
  return _ring.slice(-Math.min(count, RING_MAX));
}

function getRecentEventsByType(type, count = 50) {
  return _ring.filter(e => e.type === type).slice(-Math.min(count, RING_MAX));
}
