  ...
  try {
    ...
    for (const exec of staleExecutions.rows) {
      try {
        ...
        await db.query(
          `UPDATE posts SET status = 'OPEN', claimed_by = NULL, claimed_at = NULL WHERE id = $1 AND status = 'EXECUTING'`,
          [exec.task_id]
        );

        // Mark execution as expired
        await db.query(
          `UPDATE execution_history SET status = 'expired', error = $1 WHERE execution_id = $2`,
