...
function computeScore(id, hint, queryTerms, queryLower) {
  ...
  return { composite, semanticScore, compScore, traceMatch, verificationWeight, verificationTier, decayLabel, decayMultiplier, freshnessWeight, hallPenalty, ageDays, isVerified: verificationTier !== 'unverified', isBlacklisted, isQuarantined };
}
...
function searchLocal(query, topK = 10, opts = {}) {
  ...
  for (const [id, hint] of Object.entries(allHints)) {
    ...
    if (verifiedOnly) {
      // verified_only mode: only return sandbox_passed or above
      const allowedTiers = ['sandbox_passed', 'production_confirmed'];
      if (allowedTiers.includes(ranking.verificationTier)) {
...
function scoreFix(id, hint, relevance, rank) {
  ...
  return {
    ...
    rank_breakdown: { semantic: Math.round(rank.semanticScore * 100), component: Math.round(rank.compScore * 100), trace: Math.round(rank.traceMatch * 100), verification_weight: Math.round(rank.verificationWeight * 100), freshness_weight: Math.round(rank.freshnessWeight * 100) },
