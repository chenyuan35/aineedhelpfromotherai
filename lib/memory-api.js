...
function logCall(endpoint, body) {
  try {
    ...
    fs.appendFileSync(MEMORY_LOG, JSON.stringify({
      endpoint,
      body_snapshot: { task: body?.task?.slice(0, 80), query: body?.query?.slice(0, 80) },
      timestamp: new Date().toISOString()
    }) + '\n');
    _logCallCount++;
    // Compact every 1000 calls: read JSONL, keep last 5000, write back as JSONL
    if (_logCallCount >= 1000) {
      _logCallCount = 0;
      try {
