...
const MEMORY_LOG = path.join(__dirname, '..', 'data', 'memory-api-log.json');
const STALE_DAYS = 14; // hints older than this with low score are filtered
const MIN_SIMILARITY = 0.2; // ignore results below this match ratio

function logCall(endpoint, body) {
  try {
    const dir = path.dirname(MEMORY_LOG);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const existing = fs.existsSync(MEMORY_LOG) ? JSON.parse(fs.readFileSync(MEMORY_LOG, 'utf8')) : { calls: [] };
    existing.calls.push({ endpoint, body_snapshot: { task: body?.task?.slice(0, 80), query: body?.query?.slice(0, 80) }, timestamp: new Date().toISOString() });
    if (existing.calls.length > 10000) existing.calls = existing.calls.slice(-5000);
    fs.writeFileSync(MEMORY_LOG, JSON.stringify(existing));
  } catch { /* ignore */ }
}
...
function computeScore(id, hint, queryTerms, queryLower) {
  ...
  // Layer 1: Semantic similarity (keyword match ratio)
  ...
  // Layer 4: Verified boost
  const isVerified = hint.task_type === 'verified_resolution' || (hint.hit === true && hint.success_count > 0);
  const verifiedBoost = isVerified ? 0.15 : 0;
  ...
  const hallPenalty = isBlacklisted ? 0.5 : isQuarantined ? 0.3 : (hint.status === rc.HINT_STATUS.DECAYING && (hint.score || 0) < 0) ? 0.1 : 0;
  ...
  return { composite, semanticScore, compScore, traceMatch, verifiedBoost, freshnessWeight, hallPenalty, ageDays, isVerified, isBlacklisted, isQuarantined };
}
...
function searchLocal(query, topK = 10, opts = {}) {
  ...
  for (const [id, hint] of Object.entries(allHints)) {
    ...
    const summaryKey = (hint.solution_summary || '').slice(0, 80).toLowerCase();
    ...
    if (verifiedOnly && !isHarmful) {
      if (isFix && hint.score > 1.0 && hint.success_count > 0) {
        verifiedFixes.push(scoreFix(id, hint, relevance, ranking));
      }
      continue;
    }

    if (isFix && hint.score > 0 && !isHarmful) {
      verifiedFixes.push(scoreFix(id, hint, relevance, ranking));
    }
...
function scoreFix(id, hint, relevance, rank) {
  ...
  return {
    ...
    rank_breakdown: { semantic: Math.round(rank.semanticScore * 100), component: Math.round(rank.compScore * 100), trace: Math.round(rank.traceMatch * 100), verified_boost: rank.verifiedBoost, freshness_weight: Math.round(rank.freshnessWeight * 100) },
...
function formatRecall(results, opts = {}) {
  ...
  if (results.verified_fixes?.length > 0) {
    ...
    out += `Found verified fix from ${ago}:\n`;
    out += `\`\`\`\n${fix.summary}\n\`\`\`\n`;
    if (fix.confidence > 0) out += `(confidence: ${fix.confidence}% · ${fix.supporting_agents} agent(s) verified)\n`;
...
function getStats() {
  const log = fs.existsSync(MEMORY_LOG) ? JSON.parse(fs.readFileSync(MEMORY_LOG, 'utf8')) : { calls: [] };
  const allHints = rc.getAllHints();
  ...
  return {
    total_api_calls: log.calls?.length || 0,
...
module.exports = { submitFailure, searchMemory, submitResolution, formatRecall, getStats };
