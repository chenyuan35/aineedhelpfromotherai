...
function detectBreakagePattern(item) {
  const problem = (item.extract?.problem || '').toLowerCase();
  const logs = (item.extract?.logs || '').toLowerCase();
  const body = (item.raw_body || item.body || '').toLowerCase();
  const title = (item.title || '').toLowerCase();
  const text = [problem, logs, body, title].join(' ');
  const patterns = [];

  // version / dependency / compatibility
  if (/version\s*mismatch|incompatible\s+version|peer.?dep|requires.*but.*have|ERESOLVE|unable to resolve|conflict.*dependenc|version.*not found|doesn't work with|not compatible with|compatibility|breaking change/i.test(text)) patterns.push('version_mismatch');
  // stale / cache
  if (/stale.*cache|cache.*invalid|not updating|cache.*corrupt|clear.*cache|rebuild.*cache|incremental.*cache/i.test(text)) patterns.push('stale_cache');
  // flag / CLI
  if (/unknown option|unrecognized.*(?:flag|arg|option)|invalid.*(?:flag|option)|--\w+.*not found|no such option|unexpected argument/i.test(text)) patterns.push('hallucinated_flag');
  // deprecated / removed
  if (/deprecated|removed in|no longer supported|will be removed|has been removed|sunset|obsolete/i.test(text)) patterns.push('deprecated_api');
  // lockfile
  if (/lockfile|package-lock|yarn\.lock|pnpm-lock|npm shrinkwrap|lock file|integrity check|checksum.*mismatch|lock.*conflict/i.test(text)) patterns.push('lockfile_conflict');
  // module not found
  if (/(?:cannot\s+find|cannot\s+resolve|module\s+not\s+found|not\s+found.*module|no\s+module|could\s+not\s+import)/i.test(text)) patterns.push('missing_module');
  // timeout
  if (/timeout|timed?\s*out|request.*took too long/i.test(text)) patterns.push('timeout');
  // permission
  if (/permission|denied|EACCES|EACCESS|not authorized|forbidden|EPERM/i.test(text)) patterns.push('permission_error');
  // out of memory
  if (/out\s+of\s+memory|OOM|heap|allocation.*failed|memory.*exceeded|allocation.*failure/i.test(text)) patterns.push('out_of_memory');
  // connection / network
  if (/connection\s+(?:refused|reset|closed|lost)|ECONNREFUSED|ECONNRESET|ENOTFOUND|EAI_AGAIN|network.*error|socket.*hang|connection.*timeout/i.test(text)) patterns.push('connection_error');
  // port conflict
  if (/port.*in\s+use|address\s+already\s+in\s+use|EADDRINUSE|port.*taken/i.test(text)) patterns.push('port_conflict');
  // network generic
  if (/ETIMEDOUT|ENOTFOUND|network.*unreachable|getaddrinfo|DNS.*error|request.*fail/i.test(text)) patterns.push('network_error');
  // TLS/SSL
  if (/tls|ssl|certificate|self.signed|cert.*expir|CERT_|unable to verify|handshake.*fail/i.test(text)) patterns.push('tls_error');
  // CORS
  if (/CORS|cross.?origin|Access-Control/i.test(text)) patterns.push('cors_error');
  // config / env
  if (/(?:\.env|env\s+var|environment\s+var|no\s+(?:such\s+)?file|config.*not\s+found|configuration.*error)/i.test(text)) patterns.push('config_error');
  // token / auth
  if (/token.*(?:expir|invalid|not\s+found|required)|auth.*fail|unauthorized|401|api.?key/i.test(text)) patterns.push('auth_error');
  // build / compile
  if (/build\s+fail|compil.*error|bundle.*error|transpil.*fail|syntax.*error|tsc.*error|type.*error/i.test(text)) patterns.push('build_error');
