...
// _suppressDecay gates BOTH the interval (no new events) and the reducer
// (boot replay of old decay events is a no-op), so verify/CI leave
// resolve-cache.json untouched. Render production sets neither flag.
const _suppressDecay = process.env.VERIFY_MODE === 'true' || process.env.CI === 'true';
commitLog.on('resolve_cache/decay', (event) => {
  if (_suppressDecay) return;
  const cache = load();
  let changed = false;
...
// Periodic decay + quarantine sweep (every hour).
// _suppressDecay (declared above with the reducer) also gates this interval.
if (!_suppressDecay) {
  setInterval(() => {
