...
async function ensureTable() {
  ...
  try {
    await client.query(`
      ...
      -- Migration: backfill columns that may be missing if an older schema created the table
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS agent_name VARCHAR(128);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS task_type VARCHAR(64);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS provider VARCHAR(32);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS model VARCHAR(128);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS tokens_used INTEGER DEFAULT 0;
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS content_length INTEGER DEFAULT 0;
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS error TEXT;
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS execution_log JSONB;
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS result JSONB;
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS duration_ms INTEGER;

      -- v0.3: Runtime semantics fields (incremental migration)
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS parent_run_id VARCHAR(64);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS failure_type VARCHAR(64);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS failure_subtype VARCHAR(64);
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS evidence_refs JSONB DEFAULT '[]';
      CREATE INDEX IF NOT EXISTS idx_exec_parent_run ON execution_history(parent_run_id) WHERE parent_run_id IS NOT NULL;
      CREATE INDEX IF NOT EXISTS idx_exec_failure_type ON execution_history(failure_type) WHERE failure_type IS NOT NULL;
...
async function saveExecution(exec) {
  ...
  const db = getPool();
  if (!db) {
    console.error('[execution-history] No DB pool — cannot save execution');
    return;
  }
  await db.query(`
    INSERT INTO execution_history (
      ...
      parent_run_id, failure_type, failure_subtype, evidence_refs
    ...
    ON CONFLICT (execution_id) DO UPDATE SET
      ...
      failure_subtype = EXCLUDED.failure_subtype,
  ...
  `, [
    ...
    exec.runtime_semantics?.parent_run_id || null,
    exec.runtime_semantics?.failure_type || null,
    exec.runtime_semantics?.failure_subtype || null,
    JSON.stringify(exec.runtime_semantics?.evidence_refs || []),
  ]);
}
...
async function queryExecutions(params) {
  ...
  if (params.parent_run_id) {
    conditions.push(`parent_run_id = $${idx++}`);
    values.push(params.parent_run_id);
  }
  if (params.failure_type) {
    conditions.push(`failure_type = $${idx++}`);
  ...
  const result = await db.query(
    `SELECT execution_id, task_id, agent_id, agent_name, task_type, provider, model,
            status, tokens_used, content_length, error, created_at, completed_at, duration_ms, result,
            parent_run_id, failure_type, failure_subtype, evidence_refs
...
async function ensureMcpUsageTable() {
  ...
  try {
    await db.query(`
      ...
      -- Migration: ensure columns added in v2 exist
      ALTER TABLE mcp_usage ADD COLUMN IF NOT EXISTS ip_address VARCHAR(64);
      ALTER TABLE mcp_usage ADD COLUMN IF NOT EXISTS user_agent TEXT;
      ALTER TABLE mcp_usage ADD COLUMN IF NOT EXISTS result_hash VARCHAR(64);
    `);
...
async function getMcpUsageSummary(params = {}) {
  ...
  try {
    const [totalResult, successResult, durationResult, runtimeResult, toolResult, duplicateResult] = await Promise.all([
      ...
      db.query(`SELECT COUNT(*) as total, COUNT(DISTINCT result_hash) FILTER (WHERE result_hash IS NOT NULL) as unique_hashes FROM mcp_usage ${where}`, values)
    ]);
