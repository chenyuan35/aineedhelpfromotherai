...
async function ensureTable() {
  ...
  try {
    await client.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_exec_ip ON execution_history(ip_address);

      -- add ip_address column for existing tables (idempotent)
      ALTER TABLE execution_history ADD COLUMN IF NOT EXISTS ip_address VARCHAR(64);
...
async function saveExecution(exec) {
  ...
  await db.query(`
    INSERT INTO execution_history (
      ...
      execution_log, result, ip_address, created_at, completed_at, duration_ms
...
// Get single execution with full result (ip_address excluded — internal only)
async function getExecution(executionId) {
  await ensureTable();
  const db = getPool();
  const result = await db.query(
    `SELECT execution_id, task_id, agent_id, agent_name, task_type, provider, model,
            status, tokens_used, content_length, error,
            execution_log, result, created_at, completed_at, duration_ms
     FROM execution_history WHERE execution_id = $1`,
    [executionId]
  );
  const row = result.rows[0] || null;
  // ip_address intentionally excluded from public response
  return row;
}
...
// --- IP analytics (internal only, not exposed via API) ---
async function queryIpGrouping() {
  await ensureTable();
  const db = getPool();
  const result = await db.query(`
    SELECT ip_address,
    ...
    GROUP BY ip_address
    ORDER BY agent_count DESC, execution_count DESC
  `);
...
module.exports = {
  saveExecution, queryExecutions, getExecution, queryIpGrouping, ensureTable,
