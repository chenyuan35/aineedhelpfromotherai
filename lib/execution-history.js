...
async function logMcpUsage({ tool_name, runtime_type, agent_id, args, duration_ms, success, error_message }) {
  try {
    await ensureMcpUsageTable();
    const db = getPool();
    if (!db) return;
    await db.query(
      `INSERT INTO mcp_usage (tool_name, runtime_type, agent_id, args_json, duration_ms, success, error_message)
...
async function queryMcpUsage(params = {}) {
  ...
  const result = await db.query(
    `SELECT id, tool_name, runtime_type, agent_id, args_json, duration_ms, success, error_message, created_at
...
module.exports = {
  ...
  logMcpUsage, ensureMcpUsageTable, queryMcpUsage,
  validateSubmitResult, checkDuplicateResult
};
