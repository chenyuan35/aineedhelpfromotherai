(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=document.getElementById(`nav`);window.addEventListener(`scroll`,()=>{e.classList.toggle(`scrolled`,window.scrollY>20)},{passive:!0});var t=class{constructor(){this._v=null,this._cbs=[]}get v(){return this._v}set v(e){this._v=e,this._cbs.forEach(t=>t(e))}sub(e){return this._cbs.push(e),this._v!==null&&e(this._v),()=>this._cbs=this._cbs.filter(t=>t!==e)}},n={agents:new t,posts:new t,registered:new t,events:new t,signals:new t};async function r(e){try{let t=await fetch(e);return t.ok?await t.json():null}catch{return null}}async function i(){let e=await r(`/api/agents`);n.agents.v=e,n.registered.v=e&&e.workers||[],o(),r(`/api/posts?limit=10`).then(e=>{n.posts.v=e;let t=e&&e.data&&Array.isArray(e.data.posts)?e.data.posts:[];l(`task-count`,t.length),l(`open-count`,t.filter(e=>(e.status||``).toUpperCase()===`OPEN`).length),t.length>0&&d(t.slice(0,6))}).catch(()=>{})}i(),a();function a(){r(`/api/observed-sessions?limit=1`).then(e=>{let t=document.getElementById(`recursive-grid`);if(t)if(e&&e.sessions&&e.sessions.length>0){let n=e.sessions[0];t.innerHTML=`
      ...
      `}else t.innerHTML=`<div class="rec-empty">No observed sessions yet.</div>`})}function o(){let e=n.registered.v||[];l(`reg-count`,e.length),e.length>0&&u(e),s(),c()}async function s(){let e=document.getElementById(`fd-body`);if(e)try{let t=await r(`/api/failure-dynamics?sort=time`);if(t&&t.dynamics&&t.dynamics.length>0){e.innerHTML=t.dynamics.map(e=>{let t=e.severity===`critical`?`sev-critical`:`sev-high`,n=e.short||e.name,r=e.interventions?e.interventions.length:0;return`<div class="fd-row">
        ...
        </div>`}).join(``);return}e.innerHTML=`<div class="dyn-item"><div class="dyn-name">No dynamics</div></div>`}catch{e.innerHTML=`<div class="dyn-item"><div class="dyn-name">Failed to load</div></div>`}}function l(e,t){let n=document.getElementById(e);n&&(n.textContent=t,n.classList.remove(`skeleton`))}function u(e){let t=document.getElementById(`agent-grid`);t&&(t.innerHTML=e.map(e=>`
  ...
  `).join(``))}function d(e){let t=document.getElementById(`task-list`);!t||e.length===0||(t.innerHTML=e.map(e=>`
    <div class="task-row">
  ...
  `).join(``),document.getElementById(`task-section`)?.classList.remove(`hidden`))}var f={"resolve.hit":{icon:`✔`,cls:`hit`,label:`CACHE HIT`},"resolve.miss":{icon:`✕`,cls:`miss`,label:`CACHE MISS`},"task.claimed":{icon:`▶`,cls:`claimed`,label:`CLAIMED`},"task.submitted":{icon:`●`,cls:`submitted`,label:`SUBMITTED`},"task.created":{icon:`+`,cls:`created`,label:`CREATED`},"reasoning.stored":{icon:`◆`,cls:`stored`,label:`STORED`},root_cause_analyzed:{icon:`◎`,cls:`analyzed`,label:`ANALYZED`},behavioral_signal:{icon:`⚡`,cls:`signal`,label:`SIGNAL`}},p=document.getElementById(`obs-feed`),m=document.getElementById(`feed-empty`),h=document.getElementById(`feed-count`),g=document.getElementById(`feed-agents`),_=0,v=new Set;function y(e){let t=new Date,n=e.timestamp?new Date(e.timestamp).toTimeString().slice(0,8):t.toTimeString().slice(0,8),r=e.type||e.event_type||`event`,i=f[r]||{icon:`○`,cls:`default`,label:r.toUpperCase()},a=e.agent_id||e.agentId||`—`,o=e.task_id||e.hint_id||e.run_id||e.problem_statement||`—`;a!==`—`&&v.add(a),m&&m.remove();let s=document.createElement(`div`);s.className=`obs-event`;let c=e.narrative||`${i.label}: ${o}`,l=e.narrative_action||``;for(s.innerHTML=`
  ...
  `,p.prepend(s),requestAnimationFrame(()=>s.classList.add(`visible`)),_++,h.textContent=`${_} events`,g.textContent=`${v.size} agents`;p.children.length>100;)p.removeChild(p.lastChild)}b(),C(),T();function b(){let e=new EventSource(`/api/events`);x(`connected`),e.addEventListener(`connected`,()=>{x(`connected`)}),[`resolve.hit`,`resolve.miss`,`task.claimed`,`task.submitted`,`task.created`,`reasoning.stored`,`root_cause_analyzed`,`behavioral_signal`].forEach(t=>{e.addEventListener(t,e=>{try{y(JSON.parse(e.data))}catch{}})}),e.addEventListener(`snapshot`,e=>{try{let t=JSON.parse(e.data);t.agents&&S(t)}catch{}}),e.onerror=()=>{x(`disconnected`),e.close(),setTimeout(b,5e3)}}function x(e){let t=document.getElementById(`nav-status-dot`),n=document.getElementById(`nav-status-text`);t&&(t.className=`nav-status-dot `+e),n&&(n.textContent=e);let r=document.getElementById(`nav-status`);r&&(r.className=`nav-status `+e)}function S(e){let t=e.agents||{},n=e.memory||{},r=e.executions||{},i=(t.active||0)+(t.queued||0)+(t.running||0),a=n.total_hints||n.total||0,o=n.health_score||0,s=r.total||0,c=e=>document.getElementById(e);c(`state-agents`)&&(c(`state-agents`).textContent=i||`—`),c(`state-memory`)&&(c(`state-memory`).textContent=a||`—`),c(`state-executions`)&&(c(`state-executions`).textContent=s.toLocaleString()||`—`),c(`state-resolve-rate`)&&(c(`state-resolve-rate`).textContent=n.health_score?Math.round(o*100)+`%`:`—`),document.querySelectorAll(`.side-stat.skeleton`).forEach(e=>e.classList.remove(`skeleton`)),c(`mem-total`)&&(c(`mem-total`).textContent=a,c(`mem-active`).textContent=n.active||0,c(`mem-decaying`).textContent=n.decaying||0,c(`mem-quarantined`).textContent=n.quarantined||0)}function C(){let e=new EventSource(`/api/snapshot/live`),t=0;e.addEventListener(`snapshot`,e=>{try{let n=JSON.parse(e.data),r=n.tick||0;r!==t&&(t=r,S(n))}catch{}}),e.onerror=()=>{e.close(),setTimeout(C,15e3)}}async function w(){try{let e=await fetch(`/api/snapshot`);if(e.ok){let t=await e.json();t.snapshot&&S(t.snapshot)}}catch{}}w();function T(){let e=new EventSource(`/api/signals/live`);e.addEventListener(`signal`,e=>{try{let t=JSON.parse(e.data),n=document.getElementById(`signals-container`);if(!n)return;let r=n.querySelectorAll(`.signal-item`);r.length>=6&&r[r.length-1].remove();let i=document.createElement(`div`);i.className=`signal-item`,i.innerHTML=`
