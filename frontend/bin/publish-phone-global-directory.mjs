...
function directoryHtml(){
  ...
  const sections=markets.map(m=>{
    const rows=byMarket.get(m).map(r=>{
      ...
      return `<tr><td><a href="/tools/phone-number-survival-guide/route/${esc(r.id)}/"><strong>${esc(brand?.name||r.id)}</strong></a></td><td>${esc(statusLabel(r))}</td><td>${esc(keep)}</td><td>${Number.isFinite(r.keep?.intervalDays)?r.keep.intervalDays+'d':'—'}</td><td>${esc(r.lastVerifiedAt||'')}</td></tr>`;
    }).join('');
    return `<section class="dir-market"><h2>${esc(m)} <small>${byMarket.get(m).length} route${byMarket.get(m).length===1?'':'s'}</small></h2><table><thead><tr><th>Route</th><th>Status</th><th>Keep/yr</th><th>Interval</th><th>Verified</th></tr></thead><tbody>${rows}</tbody></table></section>`;
  }).join('\n');
  const desc=`Global SIM number retention directory: ${data.routes.length} routes across ${markets.length} markets — acquisition cost, keep-alive cost and interval, KYC and evidence status per route.`;
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Global SIM number retention directory – Phone Radar</title>
<meta name="description" content="${esc(desc)}"><meta name="robots" content="index,follow,max-image-preview:large">
<link rel="canonical" href="${canonical}"><meta property="og:title" content="Global SIM number retention directory"><meta property="og:description" content="${esc(desc)}"><meta property="og:url" content="${canonical}">
<link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/site.css">
<script id="theme-init">(()=>{try{const saved=localStorage.getItem('site-theme')||'system';const dark=window.matchMedia('(prefers-color-scheme: dark)').matches;document.documentElement.dataset.theme=saved==='system'?(dark?'dark':'light'):saved}catch{}})();</script>
<style>.dir{max-width:1000px;padding-bottom:70px}.dir h1{font-size:clamp(2rem,5vw,3.2rem);margin:.2em 0}.dir-lead{color:var(--muted);max-width:760px}.dir-market{margin:26px 0}.dir-market h2{font-size:1.15rem}.dir-market small{color:var(--muted);font-weight:400}.dir-market table{width:100%;border-collapse:collapse;font-size:.88rem}.dir-market th,.dir-market td{text-align:left;padding:7px 9px;border-bottom:1px solid var(--line)}.dir-market th{font-size:.7rem;text-transform:uppercase;letter-spacing:.05em;color:var(--muted)}</style></head>
