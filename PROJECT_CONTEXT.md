...
| Phone evidence acquisition | **WATCHER V1.1 DEPLOYED / RUNTIME VERIFIED / FIRST SIX CANDIDATES REVIEWED.** PR #211 upgraded community intelligence to hourly bounded collection and the Sep 25 runtime audit/deploy verified 4/4 sources, `candidate_total=6`, 19.3 MiB peak and active hourly scheduling after the disk-full fault was bounded. The first six candidates have now all been dispositioned in `docs/PHONE_WATCHER_CANDIDATE_REVIEW_2026-09-25.md`: Saily Switzerland data-eSIM failure/refund evidence and a Fortress haha SIM retention-price conflict were promoted as durable research packets; the iPhone IMEI question was insufficient; Ultra Mobile `$3` PayGo was already known and the seller post added no safe independent operational evidence; DMIT VPS and the V2EX beverage post were false positives. | Resolve the haha SIM `HKD 10` community keep-alive claim versus the current 3HK `HKD 100+` validity-extension language. Do not publish a keep cost until the conflict is resolved; keep storage/source health monitored because the observer remains disposable. |
...
4. resolve the Fortress haha SIM retention conflict: current community reports still describe `HKD 10` for a one-year extension while the current 3HK prepaid page says validity is extended for `HKD 100+` top-ups;
5. keep any second-observer/source-watcher host assignment in backlog until a concrete reliability/value reason exists; `codex-vps` remains Relay-history and must not be repurposed by topology alone;
6. keep Search Console measurement running through Windsor.ai, separating organic from referral/social/direct/AI traffic;
7. improve the existing index first when evidence warrants it;
8. publish a separate route detail URL only when the route has substantial unique execution content and passes the route-page gate in `docs/PHONE_RADAR_OPERATING_PLAN_2026-09-24.md`.

**Next independent session is exactly item 4.** Resolve the current haha SIM recharge/validity rule using current first-party material plus fresh independent reproduction when available. If the conflict remains unresolved without bypassing access controls, keep the route on HOLD and record the conflict rather than publishing a guessed keep cost.
