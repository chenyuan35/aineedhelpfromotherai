...
| Relay-risk pilot | PR #39 shipped `/tools/relay-exit-risk-checker/`: English community shutdown-risk and model-dilution sentiment voting, five satirical office/exit-risk animation states, prepaid-balance exposure calculator, shareable relay URLs, and links to specialist third-party model-verification tools. Votes are grouped by normalized relay hostname and stored in PostgreSQL with one updatable vote per browser-generated voter ID. PR #40 restored the reasoning handler after a Git-data upload corruption and added an explicit trailing-slash API rewrite so Vercel `/api/.../` requests reach the API tunnel instead of the static homepage. Production smoke test verified 0→1 vote, same-voter update without increasing count, aggregate/distribution changes, and GET readback; the `.invalid` smoke row was then deleted | Let real usage accumulate. Keep community percentages clearly labeled as forecasts/sentiment rather than proof of fraud, insolvency or model substitution; add evidence/history layers only when independently verifiable data exists |
...
| Utility indexing | Live sitemap has 20 URLs after the Relay Exit Risk Checker release. The existing 15-URL tracker is still 4 indexed / 11 not indexed / 0 pending / 0 errors; it predates the new relay-risk URL. Indexed tracked URLs are the homepage, `/tools/`, `/tools/cursor-usage-reset/`, and `/tools/image-compressor/`; the three utility URLs were crawled successfully by mobile Googlebot on 2026-09-13 09:57 UTC after manual request indexing. `/about/` is also indexed outside the tracker | Keep the tracker active; do not resubmit the remaining URLs repeatedly or immediately add the new relay-risk URL to manual request-indexing churn. Watch for the next crawl/index wave and first settled tool-page impressions |
...
| Homepage UX | PR #24 replaced the oversized marketing-style hero with a calmer task-first homepage: smaller headline, direct tool search, task shortcuts, spacious quick-start cards, softer visual hierarchy, and unchanged SEO URLs/canonicals. PR #39 registers the Relay Exit Risk Checker in homepage search and the tool listings without changing this layout direction | Preserve this task-first, comfort-oriented direction; avoid oversized slogans or dense dashboard-style UI |
...
| Technical quality audit | Sep 13 audit checked the then-13 live tool pages: all returned 200, were indexable, self-canonical, had one H1, valid structured data and mobile viewport metadata. The audit found a real generator defect: sitemap `lastmod` and AI-tool schema `dateModified` were tied to build time. PR #37 removed those volatile freshness claims. The Sep 14 Relay Exit Risk Checker release preserved the no-fake-freshness rule: production sitemap is now 20 URLs with 0 generated `lastmod`; the new tool is 200, self-canonical and wired to the production API. CrUX measurement remains unconfigured in GSC Wizard, so no field-performance verdict is inferred | Keep the observation window clean and do not rewrite pages from low-severity heuristics alone. For the relay-risk tool, prioritize reliability, abuse resistance and evidence quality over adding speculative scores |
...
| Last completed work | Relay Exit Risk Checker is production-live from PR #39. Release verification exposed two deployment defects before the backend was restarted: the reasoning handler had been corrupted during a Git-data upload and the apex-domain API proxy fell through to the static homepage after Vercel's trailing-slash redirect. PR #40 repaired both. CI/Eval Gate and Preview passed; Vercel production deployed; VPS main was updated to `a0e1a41`, syntax-checked and PM2-reloaded while preserving runtime data; direct tunnel and apex-domain health/stats/relay-risk endpoints return JSON; real PostgreSQL vote/upsert/readback was verified and smoke data deleted | Preserve the Sep 13 observation window for the Cursor/AIR experiment. Let the new relay-risk tool collect organic/community usage without immediately expanding it into a large directory or unverifiable risk model. After Sep 18–20, inspect the original outreach threads before deciding whether to send the prepared AIR-4 second batch |

Immediate priority: let the Sep 13 Google indexing event and PR #32 AI-discovery changes settle, keep the 15-URL tracker active, measure the Cursor breakthrough page, follow the first three outreach threads at the proper 5–7 day window, and execute the ordered AIR workstream. AIR-1 is currently blocked by Tavily quota and AIR-3 by missing metadata write access. AIR-4 remains active, with the second candidate batch researched but deliberately unsent until the first-round response window is checked. The Relay Exit Risk Checker may collect real votes in parallel, but it is not a reason to start another broad tool batch.
...
- The API tunnel reaches the historical Express/PostgreSQL runtime. Relay-risk community votes currently depend on this backend and PostgreSQL table `relay_risk_votes`.
...
- AI Relay Exit Risk Checker — community shutdown-risk/model-dilution sentiment, prepaid exposure, satirical risk-stage animation, and external specialist verification links. Shared voting uses the existing Express/PostgreSQL backend; the displayed community percentages are opinions/forecasts, not verified allegations.

The AI quota family exists because reset/credit questions are time-sensitive, high-anxiety, and can create repeat visits. Do not blindly clone every provider; validate demand and official rules first. The relay-risk pilot exists because public discussions repeatedly surface prepayment, shutdown and model-substitution anxiety; do not turn it into a broad directory until real usage and independently verifiable data justify expansion.
...
4. Let the Relay Exit Risk Checker collect real usage/votes as a separate distinct-job pilot; do not manufacture risk scores or bulk-import allegations.
