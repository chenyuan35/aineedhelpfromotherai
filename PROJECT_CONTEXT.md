...
| Phone evidence acquisition | **WATCHER V1.1 HEALTHY / GLOBE PHILIPPINES REVIEWED — HOLD.** The V2EX precision repair remains live-verified. Globe Philippines has now been reconciled in `docs/PHONE_GLOBE_PHILIPPINES_ELIGIBILITY_REVIEW_2026-09-25.md`: tourist-registered SIMs are limited to 30 days unless an approved visa extension is presented, while foreign nationals with qualifying other visa types can register without that temporary cap. Ordinary Globe Prepaid otherwise supports low-cost reload-based retention, automatic overseas roaming and free incoming SMS, including current China partner coverage, but no current end-to-end 2026 reproduction was found for a qualifying foreign national using the route long-term in mainland China. | Keep Globe backstage HOLD; deepen Holafly Always On next as a separate Data eSIM packet. |
...
10. **NEXT SESSION — Holafly Always On packet.** Verify the cheapest legitimate direct Holafly purchase that currently activates Always On, current supported-country coverage with China/Hong Kong/Singapore emphasis, device-transfer/deletion limits, actual monthly refresh behavior and current independent reliability; keep it strictly in the Data eSIM family and do not treat it as a phone-number route;
11. keep any second-observer/source-watcher host assignment in backlog until a concrete reliability/value reason exists; `codex-vps` remains Relay-history and must not be repurposed by topology alone;
12. keep Search Console measurement running through Windsor.ai, separating organic from referral/social/direct/AI traffic;
13. improve the existing index first when evidence warrants it;
14. publish a separate route detail URL only when the route has substantial unique execution content and passes the route-page gate in `docs/PHONE_RADAR_OPERATING_PLAN_2026-09-24.md`.

**Next independent session is exactly item 10.** Deepen Holafly Always On as a bounded Data eSIM packet; do not auto-publish or reopen Globe/UI work.
