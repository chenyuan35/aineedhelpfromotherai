...
| AI discovery | Public pages are crawlable. The first Exa benchmark showed an important split: direct known-URL extraction of Cursor succeeds cleanly, but semantic discovery queries do not yet return the direct utility page near the top; a GitHub result surfaced with stale legacy positioning. PR #32 adds `docs/AI_RETRIEVAL_BENCHMARK.md`, rewrites legacy `llms.txt`/`ai.txt` for the utility site, and ships those files in the static build. Tavily AI is connected but no Tavily pass/fail has been recorded yet | Verify `/llms.txt` and `/ai.txt` after PR #32 deploys, then rerun the same Exa/Tavily benchmark after crawl/discovery propagation. Continue relevant external references because current evidence points to discovery/index/ranking, not extraction, as the bottleneck |
...
| Durable workflow | `AGENTS.md`, this checkpoint, `docs/MASTER_PLAN.md`, `docs/OPERATING_WORKFLOW.md`, and `docs/AUTHORITY_AND_AI_DISCOVERY.md` are the durable handoff sources. `docs/AI_RETRIEVAL_BENCHMARK.md` is the repeatable AI-native search benchmark when agent-search visibility is being tested | Update checkpoint/master plan after material progress, update the authority/AI ledger immediately after every outreach/reply/link/citation/action, and update the retrieval benchmark after every provider test |
| Last completed work | PR #30 shipped the Cursor breakthrough page; PR #31 established durable authority/AI tracking. PR #32 then adds an AI-native retrieval benchmark and corrects stale machine-readable discovery files so future agent/search tests distinguish discovery/ranking from extraction | Verify PR #32 production files and then monitor/retest rather than adding more discovery files blindly |

Immediate priority: let the Sep 13 Google indexing event settle, keep the 15-URL tracker active, verify PR #32's `/llms.txt` and `/ai.txt` deployment, measure the Cursor breakthrough page, follow the first three outreach threads, and rerun the repeatable Exa/Tavily retrieval benchmark only after discovery changes have had time to propagate. Do not start another broad tool batch or mass outreach campaign until these signals are reviewed.
...
8. `docs/AI_RETRIEVAL_BENCHMARK.md` for repeatable Exa/Tavily/agent-search discovery and extraction tests when those providers are available.
...
5. Build and measure AI-answer visibility through crawlability, clear answers, official evidence, freshness, unique interactive value, external corroboration, and repeatable provider retrieval tests; do not guess from crawler access alone.
