...
| Phone evidence acquisition | **WATCHER V1.1 MERGED / RUNTIME DEPLOYMENT BLOCKED BY OBSERVER ACCESS.** PR #211 upgraded community intelligence to hourly bounded collection with service tags, commerce/seller-risk signals, linked public-host candidates and regression coverage; Eval Gate #709 passed. | Deploy main's v1.1 only on the actual disposable observer host once verified access is available; verify a real systemd run, source health, memory peak and next timer, then review candidates into durable research packets. |
...
| Observer access | `qwenpaw-sbs-prod-h2grp` / `qwen-control` is online but is not the Phone systemd observer host. A 2026-09-24 runtime cleanup removed stale headless Chromium, test HTTP servers and a stuck Vercel CLI process; available RAM improved from about 313 MiB to about 2.5 GiB. The host still has no swap. `codex-vps` is visible on Tailscale, but the current control path requires a fresh Tailscale SSH authorization check; two unnamed Remote Desktop Commander devices are offline and are not safely identifiable. | Keep qwen in its existing control/relay role unless deliberately reviewed for additional duties; do not infer that freed RAM changes observer identity. Resume Phone watcher deployment when the actual disposable observer host is reachable/identifiable. |
