...
| Phone data foundation | **PR #253 REBASED / CODE GATES PASS / VERCEL PREVIEW BLOCKED.** The branch is rebased on current `main` at `b10bd33`; head `5e8d038` is mergeable, Eval Gate #801 and CI #150 pass, but the fresh Vercel Preview still fails. Available Vercel credentials can access only `chenyuan19920509-5309s-projects`, while the deployment belongs to `chenyuan-s-projects`, so build logs cannot currently be inspected from the authorized project tooling. | Do not merge PR #253 until the correct Vercel scope is accessible and a real Preview succeeds; do not start importer-based route ingestion before that release gate. |
...
8. **DONE — V2EX fragment-only reply dedupe repair.** PR #225 normalized V2EX `#replyN` variants to root-thread dedupe keys; PR #226 added legacy-state migration compatibility after live verification exposed a one-time old-hash edge. Eval Gates #740/#742 passed; final observer run verified no duplicate V2EX candidate and did not suppress the source.
9. **DONE — Globe Philippines eligibility / overseas-SMS review.** `docs/PHONE_GLOBE_PHILIPPINES_ELIGIBILITY_REVIEW_2026-09-25.md` keeps Globe ordinary Prepaid at **HOLD** for general Phone Radar admission. Tourist registration remains 30-day-limited unless an approved visa extension is presented; qualifying other-visa foreign nationals can register without that temporary cap. Current retention/roaming evidence supports low-cost keep-alive and free incoming SMS abroad, including China partner coverage, but no current qualifying-foreigner end-to-end mainland-China OTP reproduction was found.
10. **DONE — Holafly Always On packet.** PR #231 (`413e6c3`) promoted it backstage as Data-eSIM only; unresolved duration/device-transfer/coverage-consistency fields and fresh 2026 independent reproduction remain re-open triggers.
...
14. **BLOCKED — normalized Phone data foundation PR #253 release.** Its two commits are rebased on current `main` (`b10bd33`) with head `5e8d038`; Eval Gate #801 and CI #150 pass and the PR is mergeable. A fresh Vercel Preview still fails, and the available Vercel session cannot access the deployment scope `chenyuan-s-projects`, so the required Preview log/green gate cannot yet be verified. Do not merge until correct-scope access is restored and a real Preview succeeds;
...
**Next independent session remains item 14 after the blocker clears.** Re-open PR #253 only when the `chenyuan-s-projects` Vercel deployment can be inspected/retried through an authorized scope; merge only after a real Preview succeeds. Do not substitute importer ingestion, bulk-merge #234–#241, or change the public Phone surface.
