...
| Last completed work | PR #48 shipped the homepage visual/IA refinement to production. The oversized split hero was replaced by a compact provider-first scene, the boss became a small transparent line-art Relay Risk entry, duplicate AI promo content was removed, and Relay Risk / generic utilities were reordered beneath the primary AI-reset navigation. The final mascot WebP is about 8 KB and is served by Vercel/CDN, not the VPS. Eval Gate and Vercel passed; production desktop and 390px Chromium checks passed and the asset returns HTTP 200 `image/webp`. Separately, the GSC Wizard indexing tracker was expanded from 15 to 16 URLs by adding Relay Exit Risk; the tracker now reports 8 indexed / 8 not indexed | Resume observation/measurement work. Preserve AIR-4 timing and Tavily quota blocker; do not use the homepage polish as a reason to start another broad product batch |
