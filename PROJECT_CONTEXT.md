...
| Phone product direction | **ACCEPTED.** Phone Radar now has three route families: **Long-term SMS/OTP numbers**, **Data SIM/eSIM routes**, and **Temporary SMS platforms**. It has two layers: **visual decision dashboard first**, **full operational guide on demand**. | Implement this accepted model on the existing canonical without waiting for another concept-approval round. |
| Phone decision-cost rule | The product must reduce user reading and decision time. Default view is a small current shortlist, not dozens of equal-weight routes and not a questionnaire. | Make the next action obvious: compare → open full guide → buy/use → setup/test → keep/recharge/recover as appropriate. |
...
| Phone global route pool | First cross-region seed spans Hong Kong, Taiwan, South Korea, Singapore, Philippines, Malaysia, New Zealand, Australia, Europe, UK, US, Canada, Thailand and more. Country is a filter/tag, not the research sequence. | After the reset ships, resume global discovery by route value rather than country coverage quota. |
