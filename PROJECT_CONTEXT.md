...
| Phone evidence acquisition | **WATCHER V1.1 HEALTHY / BATCH 2 REVIEWED / MOBAL SOURCE EVENTS RECONCILED.** The 55 records after the first six were fully dispositioned in `docs/PHONE_WATCHER_CANDIDATE_REVIEW_BATCH2_2026-09-25.md`: 2 promoted backstage research packets (Globe Philippines retention/overseas-SMS eligibility; Holafly Always On emergency data), 10 insufficient/demand-only records, 1 duplicate/restricted seller lead and 42 false positives. Live checks verified the community watcher at 4/4 HTTP 200 and the source watcher at 22 total / 16 ok-or-304 / 2 robots-skip / 4 explicit fetch blockers. The Mobal pricing/ID content-change signals were manually reconciled with no production route change. Batch 2 also confirmed 24 excess V2EX reply-fragment emissions. | Repair the bounded V2EX fragment-dedupe defect next, preserving genuinely new evidence and avoiding broad source suppression. Keep Globe/Holafly backstage until that maintenance task is complete. |
...
8. repair the bounded V2EX fragment-only reply dedupe defect while preserving genuinely new evidence and without broadly suppressing V2EX;
9. **BACKLOG — promoted packets.** Deepen Globe Philippines eligibility/overseas-SMS and Holafly Always On only after the watcher precision repair; no auto-publication;
...
**Next independent session is exactly item 8.** Repair the confirmed V2EX fragment-only reply dedupe defect in `phone-demand-watch`; preserve new evidence and do not broaden the task into source suppression or new route publication.
