...
| Phone evidence acquisition | **WATCHER V1.1 DEPLOYED / RUNTIME VERIFIED.** PR #211 upgraded community intelligence to hourly bounded collection with service tags, commerce/seller-risk signals, linked public-host candidates and regression coverage; Eval Gate #709 passed. On Sep 25, Qwen's dedicated trial-backup key authenticated successfully to the original 128 MiB / 1 GiB Debian 13 observer after the ED25519 host key was re-verified. Read-only audit found the watcher data intact but root storage at 100%, causing repeated `No space left on device` writes while systemd still reported exit 0. The state/scripts/units were backed up to Qwen, only an unused 196 MB Puppeteer Chrome download ZIP was removed, root usage fell to 82%, and GitHub `main` watcher v1.1 at `9c4fa5c` was deployed with matching checksums. A real systemd run completed 4/4 sources HTTP 200, `matched=25`, `candidate_total=6`, 19.3 MiB memory peak, and the hourly timer is enabled/active. Post-v1.1 state including `candidates.tsv` was backed up to Qwen. | Review the six current candidate records and promote only evidence-rich, deduplicated findings into durable research packets; do not publish routes/pages from watcher output alone. Keep storage/source health monitored because the observer remains disposable. |
...
3. review the six current `candidates.tsv` records into durable route/source packets with provenance, deduplication and explicit reject/insufficient outcomes;
4. keep any second-observer/source-watcher host assignment in backlog until a concrete reliability/value reason exists; `codex-vps` remains Relay-history and must not be repurposed by topology alone;
...
**Next independent session is exactly item 3.** Review the six current watcher v1.1 candidates from the off-host Qwen copy or the live observer through recovered key-only SSH, verify provenance/deduplication, and promote only evidence-rich findings into durable research packets. Do not auto-publish routes/pages or broaden crawling. If a candidate cannot be verified, record it as insufficient rather than inventing certainty.
