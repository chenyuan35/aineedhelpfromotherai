...
| Phone evidence acquisition | **WATCHER V1.1 DEPLOYED / RUNTIME VERIFIED / FIRST SIX CANDIDATES REVIEWED / HAHA RETENTION RULE RESOLVED.** PR #211 upgraded community intelligence to hourly bounded collection and the Sep 25 runtime audit/deploy verified 4/4 sources, `candidate_total=6`, 19.3 MiB peak and active hourly scheduling after the disk-full fault was bounded. The first six candidates were dispositioned in `docs/PHONE_WATCHER_CANDIDATE_REVIEW_2026-09-25.md`. Follow-up research in `docs/PHONE_HAHASIM_RETENTION_RESOLUTION_2026-09-25.md` resolved the retention-price conflict: Fortress's current product-specific FAQ says `HKD 10+` balance top-up extends haha SIM validity by 365 days and directs balance top-ups to the haha TRAVEL App; the generic 3HK online-recharge page's `HKD 100` minimum must not be applied as the haha-specific retention rule. | Keep haha SIM itself on HOLD and resolve the separate activation/real-name/onboarding-confidence blocker before any admission or public change; keep storage/source health monitored because the observer remains disposable. |
...
4. **DONE — Fortress haha SIM retention rule resolved.** Current Fortress product-specific material says `HKD 10+` balance top-up extends validity 365 days and directs current balance top-up to haha TRAVEL App; the generic 3HK web-recharge `HKD 100` floor is not the haha-specific keep-alive rule. Fresh 2026 community evidence still corroborates `HKD 10/year`, while the old 3HK-web `HKD 10` workaround remains unverified as a current official path;
5. resolve haha SIM's separate activation/real-name/onboarding-confidence blocker before considering the route admission-ready;
6. keep any second-observer/source-watcher host assignment in backlog until a concrete reliability/value reason exists; `codex-vps` remains Relay-history and must not be repurposed by topology alone;
7. keep Search Console measurement running through Windsor.ai, separating organic from referral/social/direct/AI traffic;
8. improve the existing index first when evidence warrants it;
9. publish a separate route detail URL only when the route has substantial unique execution content and passes the route-page gate in `docs/PHONE_RADAR_OPERATING_PLAN_2026-09-24.md`.

**Next independent session is exactly item 5.** Resolve the current haha SIM activation/real-name/onboarding state using current first-party material plus fresh independent reproduction where available. The retention price is no longer the blocker. Do not publish the route unless the remaining admission evidence clears the route gate.
...
- `docs/PHONE_HAHASIM_RETENTION_RESOLUTION_2026-09-25.md` — resolved haha SIM keep-alive rule and remaining route HOLD boundary.
