...
| Phone evidence acquisition | **WATCHER V1.1 HEALTHY / BATCH 2 REVIEWED / TWO NEW BACKSTAGE PACKETS.** The 55 records after the first six were fully dispositioned in `docs/PHONE_WATCHER_CANDIDATE_REVIEW_BATCH2_2026-09-25.md`: 2 promoted backstage research packets (Globe Philippines retention/overseas-SMS eligibility; Holafly Always On emergency data), 10 insufficient/demand-only records, 1 duplicate/restricted seller lead and 42 false positives. Live checks verified the community watcher at 4/4 HTTP 200 and the source watcher at 22 total / 16 ok-or-304 / 2 robots-skip / 4 explicit fetch blockers. The batch also confirmed 24 excess V2EX reply-fragment emissions. | Reconcile the current Mobal Japan pricing and ID-requirement source-change signals before new route work. Keep Globe/Holafly backstage, and queue a bounded fragment-dedupe watcher repair after Mobal; do not bulk suppress V2EX. |
...
7. reconcile the current Mobal Japan `mobal_pricing` and `mobal_id` content-change signals against current public provider pages and existing Phone facts; hash churn is only a review trigger, not proof of a durable product change;
8. **BACKLOG — watcher precision repair.** Normalize fragment-only reply URLs for dedupe while preserving genuinely new evidence; do not broadly suppress V2EX;
9. **BACKLOG — promoted packets.** Deepen Globe Philippines eligibility/overseas-SMS and Holafly Always On only after the Mobal maintenance check; no auto-publication;
10. keep any second-observer/source-watcher host assignment in backlog until a concrete reliability/value reason exists; `codex-vps` remains Relay-history and must not be repurposed by topology alone;
11. keep Search Console measurement running through Windsor.ai, separating organic from referral/social/direct/AI traffic;
12. improve the existing index first when evidence warrants it;
13. publish a separate route detail URL only when the route has substantial unique execution content and passes the route-page gate in `docs/PHONE_RADAR_OPERATING_PLAN_2026-09-24.md`.

**Next independent session is exactly item 7.** Reconcile the current Mobal Japan pricing and voice-product ID-requirement source-change events. Inspect current public content and existing accepted facts; do not infer a product change from hash differences alone.
