...
| Product direction | **STRATEGY RESET ACCEPTED 2026-09-23.** Search volume, rankings and technical polish no longer choose the product. Both official-source substitutability and independent-competitor substitutability are mandatory gates. Reset has now also been removed from primary site/discovery surfaces. Evidence: `docs/PRODUCT_DIRECTION_RESET_2026-09-23.md` and `docs/RESET_PRIMARY_SURFACE_REMOVAL_2026-09-23.md`. | Active growth work moves to Phone Radar. Reset direct URLs are preserved but no longer promoted as a primary product. Relay remains data-accrual only. |
| Phone Radar | **ACTIVE PRIMARY GROWTH PRODUCT / LIVE / VISUALLY ACCEPTED.** The accepted surface remains one canonical with three route families: Long-term SMS/OTP, Data SIM/eSIM and Temporary SMS. Community/user outcomes drive operational reality; provider pages supply commercial metadata only. Current external evidence repeatedly shows unresolved demand around real mobile numbers, OTP/SMS abroad, remote activation, app-specific verification, KYC, roaming and long-term retention. | Next bounded task: audit the existing Phone canonical's title/meta/H1/first screen and route decision fields against the accepted Phone contract. Explicitly inspect legacy `Official rules first` / `documented carrier rules` wording and historical Reset related-link leakage. No new URL or route expansion in the audit. |
| AI Reset Radar | **FROZEN / PRIMARY SURFACES REMOVED / DIRECT URLS PRESERVED.** Seven-page value classification remains: Cursor DOWNGRADE; Claude REPURPOSE; GitHub Copilot KEEP/DIFFERENTIATE; Manus DOWNGRADE; Replit DOWNGRADE; Bolt REPURPOSE; AI Credit Burn KEEP/DIFFERENTIATE. PR #195 removed Reset from homepage, Tools promotion, public AI/LLM preferred-entry hints and sitemap promotion; Eval Gate #672, Vercel Preview and apex production verification passed. | No Cursor-first optimization, no Q-015, no new generic reset pages and no generic Codex reset tracker. Any future Reset/Codex work requires a separately proven non-duplicative user job. |
...
| Search Console | Windsor.ai Search Console is the current in-chat measurement path; official Google Search Console API/export is the fallback. Cursor is exposed but not clicked; Phone is indexed but lacked exact-page rows in the latest accepted window. | Use GSC after product-value/competition gates, not as the product selector. Phone's lack of exposure no longer blocks the bounded contract audit. |
...
2. Reset has been removed from primary homepage/Tools/AI-discovery/sitemap promotion; old Reset URLs remain directly reachable only while retirement/repurpose decisions stay separate.
...
The next task is audit-only on `/tools/phone-number-survival-guide/` plus the Phone-facing homepage copy that frames it. Validate the accepted contract rather than inventing a new one: three fixed families, visual decision dashboard first, Full guide on demand, community/forum/current-user outcomes as operational reality, provider pages only for commercial metadata. Explicitly audit the reproduced legacy homepage phrases `Official rules first` / `documented carrier rules` and the Phone page's historical Reset related-tool link. End with one bounded defect/change queue. Do not add another route/page or deploy in that audit session.
...
- `docs/RESET_PRIMARY_SURFACE_REMOVAL_2026-09-23.md` — PR #195 scope, verification and production result for removing Reset from primary product/discovery surfaces while preserving direct URLs.
